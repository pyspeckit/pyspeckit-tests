...
def test_convert_to_unit():
	run_with_assert = False
	for from_unit, to_unit, _, __ in u.doppler_optical(1*u.Hz):
		sp = SpectroscopicAxis(np.linspace(-100,100,100), unit=from_unit, refX=23.2*u.Hz, velocity_convention='optical')
	...
	for from_unit, to_unit, _, __ in u.doppler_relativistic(1*u.Angstrom):
		sp = SpectroscopicAxis(np.linspace(-100,100,100), unit=from_unit, refX=23.2*u.Hz, velocity_convention='relativistic')
		sp.convert_to_unit(to_unit)
		print sp
		if(run_with_assert):
			assert sp.unit == to_unit
